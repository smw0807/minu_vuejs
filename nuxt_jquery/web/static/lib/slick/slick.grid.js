...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    function getHeadersWidth() {
      ...
      return (parseInt(Math.max(headersWidth, viewportW)) + 3000);
    }
    ...
    function createColumnHeaders() {
      ...
      for (var i = 0; i < columns.length; i++) {
		...
		// 통합검색에서 코드화 변환 부분. 통합검색 페이지에서만 show
		if (m.type == "code_s" || m.type == "code_i") {
			header.prepend("<div class='column_type_code' style='display: none;'><input type='checkbox' id='code_" + m.id + "'><label for='code_" + m.id + "'></label></div>");
		}
...
*/      
    ...
    function appendRowHtml(stringArray, row, range) {
	  ...
	  if (options.threat_pressing) {
		  if(d.POL_IS_PRESSING == true || d.POL_IS_PRESSING == 'true') {
			  stringArray.push("<div class='ui-widget-content " + rowCss + "' style='top:" + getRowTop(row) + "px; background-color:rgba(255, 0, 0, 0.3);'>");
		  } else {
    ...
    function resizeCanvas() {
      ...
      if (options.autoHeight) {
        ...
        viewportH = getViewportHeight();
			...
			viewportH = viewportH - options.heightAdjm;
    ...
    function renderRows(range) {
      ...
      for (var i = 0, ii = rows.length; i < ii; i++) {        
        ...
        parentNode.appendChild(rowsCache[rows[i]].rowNode);
      }
