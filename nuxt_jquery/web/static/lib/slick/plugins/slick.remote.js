(function ($) {
  ...
  function RemoteModel() {
    ...
    function ensureData(from, to) {
      ...
      var url = req_url + "&start=" + (fromPage * PAGESIZE) + "&limit=" + (((toPage - fromPage) * PAGESIZE) + PAGESIZE);;

      if (sortcol != null) {
        //url += ("&sortby=" + sortcol + ((sortdir > 0) ? "+asc" : "+desc"));
        url += ("&sort=" + sortcol +"&sortdir="+ ((sortdir > 0) ? "asc" : "desc"));
      }
      ...
      h_request = setTimeout(function () {
        ...
        $.postJSON(url, {}, function(rt, status){
          if(status === 'success'){
            if(rt.sError) {
              alert(rt.sError);
            } else {
    ...
    function onSuccess(resp) {
      ...
      data.length = parseInt(resp.hits); // limitation of the API
