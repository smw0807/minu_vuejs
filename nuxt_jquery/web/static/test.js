...
define(['../_module'], function (controllers) {
	controllers.controller('MonitoringCtrl', function ($scope, $rootScope, $http, $element, $interval, $filter, $compile, $interpolate, cfpLoadingBar) {
		...
		//@@EXP 수집일시 데이터포맷을 보기 편하게 변환
		// var view_time = function (r, c, v, def, ctx) {
			// var text1 = v.replace("T", " ");
			// text1 = text1.replace("+0900", "");
			// return text1;
		// };
		//@@EXP 위협경보 분석창
		// var fmtScenario = function (r, c, v, def, ctx) {
		// 	var val = v;
		// 	if (NI_CONF.is_ecsc == 'N') {
		// 		if (ctx.INST_DETAIL_NM == '' || typeof ctx.INST_DETAIL_NM == 'undefined') {
		// 			val = v;
		// 		} else {
		// 			val =  ctx.INST_DETAIL_NM;
		// 		}
		// 	}
		// 	if (v) {
		// 		val = '<a href="javascript:;" class="show_threat" ng-click="pop_edit(\'' + r + '\')" style="color:#337ab7;">' + val + '</a>';
		// 	}
		// 	return val;
		// };
		...
		// var risk_rank = function (r, c, v, def, ctx) {
		// 	console.log(ctx);
		// 	var result = '';
		// 	if (!risk_code) {
		// 		window.location.reload();
		// 	}
		// 	if (typeof ctx.RISK_V2 == 'undefined') {
		// 		try {
		// 			if (parseInt(ctx.RISK.RISK_VAL_0) <= 20) {
		// 				result = '<span style="color:#1DDB16;">' + "매우낮음" + '</span>';
		// 			} else if (parseInt(ctx.RISK.RISK_VAL_0) >= 21 && parseInt(ctx.RISK.RISK_VAL_0) <= 40) {
		// 				result = '<span style="color:#0100FF;">' + "낮음" + '</span>';
		// 			} else if (parseInt(ctx.RISK.RISK_VAL_0) >= 41 && parseInt(ctx.RISK.RISK_VAL_0) <= 60) {
		// 				result = '<span style="color:#EDD200;">' + "보통" + '</span>';
		// 			} else if (parseInt(ctx.RISK.RISK_VAL_0) >= 61 && parseInt(ctx.RISK.RISK_VAL_0) <= 80) {
		// 				result = '<span style="color:#FF5E00;">' + "높음" + '</span>';
		// 			} else if (parseInt(ctx.RISK.RISK_VAL_0) >= 81 && parseInt(ctx.RISK.RISK_VAL_0) <= 100) {
		// 				result = '<span style="color:red;">' + "매우높음" + '</span>';
		// 			}
		// 			return result;
		// 		} catch (err) {
		// 			console.log(err);
		...
		// 	} else {
		// 		try {
		// 			if (parseInt(ctx.RISK_V2.RISK_VAL_0) <= 20) {
		// 				result = '<span style="color:#1DDB16;">' + "매우낮음" + '</span>';
		// 			} else if (parseInt(ctx.RISK_V2.RISK_VAL_0) >= 21 && parseInt(ctx.RISK_V2.RISK_VAL_0) <= 40) {
		// 				result = '<span style="color:#0100FF;">' + "낮음" + '</span>';
		// 			} else if (parseInt(ctx.RISK_V2.RISK_VAL_0) >= 41 && parseInt(ctx.RISK_V2.RISK_VAL_0) <= 60) {
		// 				result = '<span style="color:#EDD200;">' + "보통" + '</span>';
		// 			} else if (parseInt(ctx.RISK_V2.RISK_VAL_0) >= 61 && parseInt(ctx.RISK_V2.RISK_VAL_0) <= 80) {
		// 				result = '<span style="color:#FF5E00;">' + "높음" + '</span>';
		// 			} else if (parseInt(ctx.RISK_V2.RISK_VAL_0) >= 81 && parseInt(ctx.RISK_V2.RISK_VAL_0) <= 100) {
		// 				result = '<span style="color:red;">' + "매우높음" + '</span>';
		// 			}
		// 			return result;
		// 		} catch (err) {
		...
		//@@EXP 장비유형 출력 
		// var gear_type = function (r, c, v, def, ctx) {
		// 	switch (v) {
		// 		case '0':
		// 			return 'NTM';
		// 			break;
		...
		//@@EXP 공격유형 코드
		// var att_types = function (r, c, v, def, ctx) {
		...
		//@@EXP 관제모니터링 //협의하에 관제모니터링 컬럼 제거
		// var in_spec_2_3 = function (r, c, v, def, ctx) {
		// 	var data = 0;
		// 	if (typeof ctx.RISK_V2 == 'undefined') {
		// 		if (ctx.RISK.IN_SPEC_VAL_2_3_O) {
		// 			data = ctx.RISK.IN_SPEC_VAL_2_3_O.toFixed(1);
		// 		} else {
		// 			data = ctx.RISK.IN_SPEC_VAL_2_3.toFixed(1);
		// 		}
		// 	} else {
		// 		//todo RISK_V2 일때 넣을 데이터 정해줘야함
		// 		data = '';
		// 	}
		// 	return data;
		// };
		//EXP@@ 시나리오
		// var in_spec_2_4 = function (r, c, v, def, ctx) {
		// 	var data = 0;
		// 	if (typeof ctx.RISK_V2 == 'undefined') {
		// 		if (ctx.RISK.IN_SPEC_VAL_2_4_O) {
		// 			data = ctx.RISK.IN_SPEC_VAL_2_4_O.toFixed(1);
		// 		} else {
		// 			data = ctx.RISK.IN_SPEC_VAL_2_4.toFixed(1);
		// 		}
		// 	} else {
		// 		//todo RISK_V2 일때 넣을 데이터 정해줘야함
		// 		if (typeof ctx.POL_HIT_RATE2 == 'undefined') {
		// 			data = '';
		// 		} else {
		// 			data = ctx.POL_HIT_RATE2.toFixed(1);
		// 		}
		// 	}
		// 	if (data == 0) {
		// 		data = 0;
		...
		//@@EXP 프로토콜 MTM일 떄는 출력안하게 바꿔주는거
		// var proto_type = function (r, c, v, def, ctx) {
		// 	if (ctx.GEAR_TYPE == "0") {
		// 		return v;
		// 	} else {
		// 		return "-";
		...
		//@@EXP 시나리오 건수 표기
		//var scen_count = function (r, c, v, def, ctx) {
		//	return ctx.SCEN_INFOS.length + "건";
		//};
		//@@EXP 자산구분 표기
		// var assets_check = function (r, c, v, def, ctx) {
		// 	var rs = '-';
		// 	var result = [];
		// 	if (v != null) {
		// 		console.log("if 문");
		// 		//예전꺼
		// 		for (var i = 1; i < 23; i++) {
		// 			if (v["ASSETS_VAL_" + i] != 0) {
		// 				result.push({name: assets_name['ASSETS_VAL_' + i], value: v['ASSETS_VAL_' + i]});
		// 				rs = assets_name['ASSETS_VAL_' + i];
		// 			}
		// 		}
		// 	} else {
		// 		console.log("else 문");
		// 		//새로운 위험도
		// 		for (var i = 1; i < 23; i++) {
		// 			if (ctx.RISK_V2["ASSETS_VAL_" + i] != 0) {
		// 				result.push({name: assets_name['ASSETS_VAL_' + i], value: ctx.RISK_V2['ASSETS_VAL_' + i]});
		// 				rs = assets_name['ASSETS_VAL_' + i];
		// 			}
		// 		}
		// 	}
		...
		// };
		//@@EXP 해외아이피 표기
		// var ct_ip_check = function (r, c, v, def, ctx) {
		// 	var rs = '';
		// 	var no_kr = true;
		// 	var att_ct_code = ctx.TW_ATT_CT_CODE;
		// 	var dmg_ct_code = ctx.TW_DMG_CT_CODE;
		// 	var src_chk = false;
		// 	var dst_chk = false;
		// 	if (att_ct_code == 'KR' || att_ct_code == '00') {
		// 	} else {
		// 		src_chk = true;
		// 		no_kr = false;
		// 	}
		// 	if (dmg_ct_code == 'KR' || dmg_ct_code == '00') {
		// 	} else {
		// 		dst_chk = true;
		// 		no_kr = false;
		// 	}
		...
		// 		rs = '-';
		// 	} else {
		// 		if (src_chk == true && dst_chk == true) {
		// 			rs = 'Src/Dst';
		// 		} else if (dst_chk == true) {
		// 			rs = 'Dst';
		// 		} else  if (src_chk == true) {
		// 			rs = 'Src';
		// 		} else {
		// 			rs = '-';
		// 		}
...
//			filter_list.risk = el.find("#risk_filter").val();
		...
		tbl.threat_list.onDblClick.subscribe(function (e, args) {
			...
			// 		if (obj.id == 'TW_ATT_CT_CODE') return;
			// 		if (obj.id == 'GEAR_TYPE') {
			// 			val = val == '0' ? 'NTM' : 'MTM';
			// 		}
			// 		//  if (obj.id == 'DRULE_ATT_TYPE_CODE1') {
			// 		//  	for (var i in att_code) {
			// 		//  		if (att_code[i].CC_CODE == val) {
			// 		// 			val = att_code[i].CC_NM + " ( " + val + " )";
			// 		//  		}
			// 		//  	}
			// 		// }
			// 		if (obj.id == 'DRULE_ATT_TYPE_CODE1') {
			// 			var val_code = NIB_CODE_LIST('att_type');
			// 			if (val_code.get(val).CODE == val) {
			// 				console.log("asdasdasd?");
			// 				   val = val_code.get(val).NAME + " ( " + val + " )";
			// 			}
			// 	   }else if (obj.id == 'SCEN_INFOS') {
			// 			var text = '';
			// 			for (var i in val) {
			// 				text += val[i].POL_NM + "\n";
			// 			}
			...
			// 			var temp = 0;
			// 				for(var i = 1; i < 24; i++) {
			// 					if(temp < data.RISK_V2['ASSETS_VAL_'+ i]){
			// 						temp = data.RISK_V2['ASSETS_VAL_'+ i];
			// 						val = assets_name['ASSETS_VAL_' + i];
			// 					}
			// 				}
			// 				if(temp == 0){
			// 					return;
			// 				}
			// 		}
			// 		alert('클릭한 필드 : [' + obj.name + ']\n데이터:\n' + val);
			// 	}
			// 	if (obj.id == 'RISK.RISK_VAL_0' || obj.id == 'RISK.IN_SPEC_VAL_2_3' || obj.id == 'RISK.IN_SPEC_VAL_2_4') {
			// 		var text = '';
			// 		// if (parseInt(data.RISK.RISK_VAL_0) <= 20) {
			// 		// 	text = risk_code[0].CC_NM;
			// 		// } else if (parseInt(data.RISK.RISK_VAL_0) >= 21 && parseInt(data.RISK.RISK_VAL_0) <= 40) {
			// 		// 	text = risk_code[1].CC_NM;
			// 		// } else if (parseInt(data.RISK.RISK_VAL_0) >= 41 && parseInt(data.RISK.RISK_VAL_0) <= 60) {
			// 		// 	text = risk_code[2].CC_NM;
			// 		// } else if (parseInt(data.RISK.RISK_VAL_0) >= 61 && parseInt(data.RISK.RISK_VAL_0) <= 80) {
			// 		// 	text = risk_code[3].CC_NM;
			// 		// } else if (parseInt(data.RISK.RISK_VAL_0) >= 81 && parseInt(data.RISK.RISK_VAL_0) <= 100) {
			// 		// 	text = risk_code[4].CC_NM;
			// 		// }
			// 		var risk_type = 'RISK';
			// 		if (data.RISK == null) {
			// 			risk_type = 'RISK_V2';
			// 		}
			// 		if (obj.id == 'RISK.RISK_VAL_0') {
			// 			val = data[risk_type].RISK_VAL_0 == null ? data.RISK_V2.RISK_VAL_0 : data[risk_type].RISK_VAL_0;
			// 			alert('클릭한 필드 : [' + obj.name + ']\n데이터: ' + text + " ( " + val + " )");
			// 		}
			// 		if (obj.id == 'RISK.IN_SPEC_VAL_2_3') {
			// 			//todo RISK_V2 일때 넣을 데이터 정해줘야함
			// 			val = data[risk_type].IN_SPEC_VAL_2_3;
			// 			alert('클릭한 필드 : [' + obj.name + ']\n데이터: ' + val);
			// 		}
			// 		if (obj.id == 'RISK.IN_SPEC_VAL_2_4') {
			// 			//todo RISK_V2 일때 넣을 데이터 정해줘야함
			// 			if (risk_type == 'RISK_V2') {
			// 				val = data.POL_HIT_RATE2;
			// 			} else {
			// 				val = data[risk_type].IN_SPEC_VAL_2_4;
			// 			}
			// 			alert('클릭한 필드 : [' + obj.name + ']\n데이터: ' + val);
			// 		}
			// 	} 
